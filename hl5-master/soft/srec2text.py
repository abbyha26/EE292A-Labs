...
def main():
    ...
    try:
        ...
        print(f"Error: failed to open {in_path}: {e}")
        sys.exit(1)
...
def srec_to_text(fd):
    ...
    for raw in fd:
        ...
        # Termination records (S7/S8/S9): skip
        if s_type in (7, 8, 9):
            continue

        # Check for contiguous addressing
        if current_address != 0 and address != current_address:
            print(f'Error: SREC skipping addresses -> "{entry}"')
            sys.exit(1)
        ...
        for i in range(words):
            ...
            # Convert little-endian byte order (AABBCCDD) to big-endian (DDCCBBAA)
