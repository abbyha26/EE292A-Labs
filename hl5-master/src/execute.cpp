...
EXE_BODY:
    while(true) {
        ...
        // Get
        input = din.get();
        cout << "input.rs2 start" << input.rs2 <<endl;
        ...
        sc_int<XLEN> temp1, temp3a, temp3b;
        sc_uint<XLEN> temp2, temp4a, temp4b;
        //sc_biguint<XLEN*2+2> temp2big, temp4abig, temp4bbig;
        ...
        if (input.alu_src == (sc_bv<ALUSRC_SIZE>)ALUSRC_RS2) {
            tmp_rs2 = input.rs2;
            cout<< "temp_rs2 br1= " << (sc_int<XLEN>)tmp_rs2<< endl;
        } else if (input.alu_src == (sc_bv<ALUSRC_SIZE>)ALUSRC_IMM_I) {
            tmp_rs2 = tmp_sigext_imm_i;
            cout<< "temp_rs2 br2= " << (sc_int<XLEN>)tmp_rs2<< endl;
            ...
            cout<< "temp_rs2 br3= " << (sc_int<XLEN>)tmp_rs2<< endl;
        } else {
            // ALUSRC_IMM_U
            tmp_rs2 = tmp_zerofill_imm_u;
            cout<< "temp_rs2 br4= " << (sc_int<XLEN>)tmp_rs2<< endl;
        }
        cout<< "temp_rs2 br5= " << (sc_int<XLEN>)tmp_rs2<< endl;
        // ALU body
        ...
        case ALUOP_DIV: // DIV calls div_func
            ...
            //cynw::div<XLEN,XLEN>((sc_int<XLEN>)input.rs1, (sc_int<XLEN>)tmp_rs2, &((sc_int<XLEN>)div_res.quotient));
    #ifdef DPA_CYNDIV
        {
            div((sc_int<XLEN>)input.rs1, (sc_int<XLEN>)tmp_rs2, temp1, DIV_LOOP::CYNW_BREAK,DIV_STYLE::CYNW_DPOPT);
            div_res.quotient = temp1;
        }
    #elif DIV_RADIX_2
        {
            div((sc_int<XLEN>)input.rs1, (sc_int<XLEN>)tmp_rs2, temp1, DIV_LOOP::CYNW_UNROLL, DIV_STYLE::CYNW_RAD2);
             div_res.quotient = temp1;
        }
    #elif DIV_RADIX_4
        {
            div((sc_int<XLEN>)input.rs1, (sc_int<XLEN>)tmp_rs2, temp1, DIV_LOOP::CYNW_UNROLL, DIV_STYLE::CYNW_RAD4);
            //cynw::div_radix4((sc_int<XLEN>)input.rs1, (sc_int<XLEN>)tmp_rs2, temp1);
            div_res.quotient = temp1;
        }
    #else
        {
                    HLS_DPOPT_REGION(0,"div");
                    cout<< "rs1= " << (sc_int<XLEN>)input.rs1 << endl;
                    cout<< "temp_rs2= " << (sc_int<XLEN>)tmp_rs2<< endl;
                    div_res.quotient = (sc_int<XLEN>)input.rs1 / (sc_int<XLEN>)tmp_rs2;
        }
    #endif
        ...
        case ALUOP_DIVU: // DIVU calls udiv_func
            ...
            //cynw::div<XLEN,XLEN>((sc_uint<XLEN>)input.rs1, (sc_uint<XLEN>)tmp_rs2, &((sc_uint<XLEN>)u_div_res.quotient));
    #ifdef DPA_CYNDIV
        {
               div((sc_uint<XLEN>)input.rs1, (sc_uint<XLEN>)tmp_rs2, temp2, DIV_LOOP::CYNW_BREAK, DIV_STYLE::CYNW_DPOPT);
                u_div_res.quotient = temp2;
        }
    #elif DIV_RADIX_2
        {
                div((sc_uint<XLEN>)input.rs1, (sc_uint<XLEN>)tmp_rs2, temp2, DIV_LOOP::CYNW_UNROLL, DIV_STYLE::CYNW_RAD2);
                u_div_res.quotient = temp2;
        }
    #elif DIV_RADIX_4
        {
            div((sc_uint<XLEN>)input.rs1, (sc_uint<XLEN>)tmp_rs2, temp2, DIV_LOOP::CYNW_UNROLL, DIV_STYLE::CYNW_RAD4);
            u_div_res.quotient = temp2;
        }
    #else
        {
                    HLS_DPOPT_REGION(0,"div");
                    u_div_res.quotient = (sc_uint<XLEN>)input.rs1 / (sc_uint<XLEN>)tmp_rs2;
        }
    #endif
           //cynw::div<XLEN,XLEN>((sc_uint<XLEN>)input.rs1, (sc_uint<XLEN>)tmp_rs2, &(u_div_res.quotient));
    ...
    case ALUOP_REM: // REM calls div_func
            //div_res = div_func((sc_int<XLEN>)input.rs1, (sc_int<XLEN>)tmp_rs2);
            //cynw::div<XLEN,XLEN>((sc_int<XLEN>)input.rs1, (sc_int<XLEN>)tmp_rs2, &(div_res.quotient),&(div_res.remainder));
    #ifdef DPA_CYNDIV
         {
                //sc_int<XLEN> temp3a, temp3b;
                div((sc_int<XLEN>)input.rs1, (sc_int<XLEN>)tmp_rs2, temp3a, temp3b, DIV_LOOP::CYNW_BREAK,DIV_STYLE::CYNW_DPOPT);
                div_res.quotient = temp3a;
                div_res.remainder = temp3b;
         }
    #elif DIV_RADIX_2
         {
            div((sc_int<XLEN>)input.rs1, (sc_int<XLEN>)tmp_rs2, temp3a, temp3b, DIV_LOOP::CYNW_UNROLL, DIV_STYLE::CYNW_RAD2);
            div_res.quotient = temp3a;
            div_res.remainder = temp3b;
        }
    #elif DIV_RADIX_4
        {
            div((sc_int<XLEN>)input.rs1, (sc_int<XLEN>)tmp_rs2, temp3a, temp3b, DIV_LOOP::CYNW_UNROLL, DIV_STYLE::CYNW_RAD4);
            div_res.quotient = temp3a;
            div_res.remainder = temp3b;
        }
    #else
         {
                    HLS_DPOPT_REGION(0,"div");
                    div_res.remainder = (sc_uint<XLEN>)input.rs1 % (sc_uint<XLEN>)tmp_rs2;
         }
    #endif
           //cynw::div<XLEN,XLEN>((sc_uint<XLEN>)input.rs1, (sc_uint<XLEN>)tmp_rs2, &(u_div_res.quotient));
    ...
    case ALUOP_REMU: // REMU calls udiv_func
            //u_div_res = udiv_func((sc_uint<XLEN>)input.rs1, (sc_uint<XLEN>)tmp_rs2);
            //cynw::div<XLEN,XLEN>((sc_uint<XLEN>)input.rs1, (sc_uint<XLEN>)tmp_rs2, &(u_div_res.quotient), &(u_div_res.remainder));
    #ifdef DPA_CYNDIV
        {
            //sc_uint<XLEN> temp4a, temp4b;
            div((sc_uint<XLEN>)input.rs1, (sc_uint<XLEN>)tmp_rs2, temp4a, temp4b, DIV_LOOP::CYNW_BREAK,DIV_STYLE::CYNW_DPOPT);
            u_div_res.quotient = temp4a;
            u_div_res.remainder = temp4b;
        }
    #elif DIV_RADIX_2
        {
            div((sc_uint<XLEN>)input.rs1, (sc_uint<XLEN>)tmp_rs2, temp4a, temp4b, DIV_LOOP::CYNW_UNROLL, DIV_STYLE::CYNW_RAD2);
            u_div_res.quotient = temp4a;
            u_div_res.remainder = temp4b;
        }
    #elif DIV_RADIX_4
        {
            div((sc_uint<XLEN>)input.rs1, (sc_uint<XLEN>)tmp_rs2, temp4a, temp4b, DIV_LOOP::CYNW_UNROLL, DIV_STYLE::CYNW_RAD4);
            u_div_res.quotient = temp4a;
            u_div_res.remainder = temp4b;
        }
    #else
        {
              HLS_DPOPT_REGION(0,"div");
              u_div_res.remainder = (sc_uint<XLEN>)input.rs1 % (sc_uint<XLEN>)tmp_rs2;
        }
    #endif
           //cynw::div<XLEN,XLEN>((sc_uint<XLEN>)input.rs1, (sc_uint<XLEN>)tmp_rs2, &(u_div_res.quotient));
