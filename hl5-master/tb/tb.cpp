...
void tb::source()
{
	...
	//
	//	Open the *.txt file that contains the program to run on the processor.
	//
    dbg_prog.open(esc_argv(1), std::ifstream::in);
	//
	//	Check that the file open was successful.
	//
	if (!dbg_prog.is_open()) {
		std::cout << "ERROR: Failed to open input file <" << esc_argv(1) << ">" << std::endl;
		esc_stop();
	}
    unsigned index;
    unsigned address;
    ...
    while (dbg_prog >> std::hex >> address) {
		//
		//	This does not catch the case where there is some kind of problem in the file.
		//	For example, there may be a line starting with "Error:....". We need to build
		//	in some checking on the file.
		//
		//cerr << "LOADING ADDRESS: 0x" << hex << address << "\n";
